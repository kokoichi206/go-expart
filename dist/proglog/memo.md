...
### メトリクス
何件のリクエストが失敗したのか、各リクエストの処理時間など、時間と共に数値データを測定したもの。
システムの健全性を報告、内部アラート、ダッシュボードによるグラフ化。

メトリクスは数値データなので、徐々に分解能を下げていくことで、必要なストレージ容量や検索に要する時間を減らせる。
...
ログの取りすぎに注意し、学習しながら役立たないログを減らしていくこと！

### トレース

リクエストのライフサイクルを捉え、システムを流れるリクエストを追跡できる。
トレースに詳細な情報をタグづけすることで、各リクエストの詳細を知ることができる。一般的な例は、各トレースにユーザ ID をタグづけすることで、あるユーザに問題が発生した場合、そのユーザのリクエストを容易に見つけられること。

トレースは、１つ以上のスパンで構成される。各スパンは、リクエストの実行の一部を表す。

### サービス

サービスを本番環境にデプロイする際には通常、Prometheus, Elasticsearch, Jaeger のような外部サービスに送信するように設定する。
ここでは簡単にするためにファイルに記録する。
