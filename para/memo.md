...
## goroutine
ゴルーチンはGoのプログラムでの最も基本的な構成単位。全てのGoのプログラムには最低一つのメインゴルーチンがある。

ゴルーチンとは他のコードに対し並行に実行している関数のこと。（必ずしも並列ではない）ゴルーチンは go キーワードを関数呼び出しの前におくことで簡単に起動できる。

go キーワードは無名関数に対しても動作する。

ゴルーチンは（他の言語にも似た並行処理のプリミティブは存在するが）Go特有のもの。ゴルーチンはOSスレッドではなく、また必ずしもグリーンスレッドー言語のランタイムにより管理されるスレッドーではない。ゴルーチンは**コルーチン（coroutine）**として知られる高水準の抽象化。ゴルーチンは単に「プリエンプティブでない」並行処理のサブルーチンである。つまり、割り込みをされることがない、ということ。

ゴルーチンは特殊な子ルーチンであり、Goのランタイムと密結合している。

こルーチン、また結果としてゴルーチンは、暗黙的には並行処理の構成要素だが、並行性というのはコルーチンの性質ではない。並行である場合には、何かが複数のこルーチンを同時管理して、それぞれに実行の機会を与える必要

Go がゴルーチンをホストする機構は、いわゆ M:N スケジューラーとよばれる実装になっている。M個のグリーンスレッドをN個のOSスレッドに対応させるもの。

fork-joinモデルでの並行処理。

ゴルーチンは作られたアドレスと同じ空間で実行する。

ゴルーチンはお互いに同じアドレス空間を操作していて、単純に関数をホストしているため、ゴルーチンを使うことは並行でないコードを書くことの自然な延長になっている。

struct{} は空構造体、メモリを消費しない

### sync パッケージ

#### WaitGroup
ひとまとまりの並行処理があったとき、その結果を気にしない、もしくは他に結果を収集する手段がある場合に、それらの処理の完了を待つ手段として非常に有効。どちらの前提も当てはまらない場合には、select 文を使う。

WaitGroup を並行処理で安全なカウンターと考えることができる。Add の呼び出しは監視対象のゴルーチンの外で行うこと！こうしないと競合状態が生じる。。。

#### Mutex と RWMutex
"mutual exclusion" の略で、プログラム内のクリティカルセクションを保護する方法の１つ。クリティカルセクションは、プログラムが共有リソースに対する排他的アクセスを必要とする場所のこと。
Mutexは並行処理で安全な方法でこれらの共有リソースに対する排他的アクセスを提供している。チャネルは通信によってメモリを共有し、Mutexは開発者が守らなければならないメモリに対する同期アクセスの習慣を作ることでメモリを共有する。
...
### チャネル（channel）
チャネルは Hoare の CSP に由来する、Go における同期処理のプリミティブの１つ。メモリに対するアクセスを同期するのに使える一方で、ゴルーチン間の通信に使うのが最適。

情報の流れの水路として機能。

一般的には Ch, c などと命名することが多い

``` sh
# suffix に Ch
$ grep -r -E "[A-Za-z]+Ch " $(go env GOROOT)/src | wc -l
```

チャネルを初期化せずに close を行うと、デッドロックではなく、純粋にパニックになる！ヤバみ

どのゴルーチンがチャネルを所有しているかをはっきりさせることが重要。その上で、チャネルの所有権のスコープをできる限り小さくする！

### select 文
select 文はチャネルをまとめる糊。これによりプログラム内でより大きな抽象化をするためにチャネルを組み合わせられる。
