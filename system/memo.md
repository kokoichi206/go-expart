...
現在広く利用されているコンテナ技術である docker のコアは、Go 言語性の libcontainer というライブラリ！

- https://github.com/opencontainers/runc/tree/main/libcontainer

### 仮想化

- コンテナより先に使われるようになった技術
- ホスト OS の上にハードウェアをエミュレートする仮想化ソフトを作り、その上にゲスト OS をインストール
- 2つに大別
  - エミュレーション
  - ネイティブ仮想化
- OS の上で OS を動かす
...
- OS レベル仮想化
- OS が持つリソースに「壁」を作ることで、そのプロセス専用の環境を作る
- ネットワーク、ファイルシステム、プロセス、並列処理、メモリ
